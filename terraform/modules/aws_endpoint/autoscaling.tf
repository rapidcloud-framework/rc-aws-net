resource "aws_vpc_endpoint" "autoscaling_endpoint" {
  count             = var.service == "autoscaling" ? 1 : 0
  vpc_id            = var.vpc_id
  service_name      = "com.amazonaws.${var.region}.autoscaling"
  vpc_endpoint_type = "Interface"
  subnet_ids        = [for subnet, id in var.subnets : id]
  security_group_ids = [
    aws_security_group.autoscaling_endpoint[0].id
  ]
  private_dns_enabled = true
  tags                = merge(local.rc_tags, var.tags)
}

resource "aws_security_group" "autoscaling_endpoint" {
  count  = var.service == "autoscaling" ? 1 : 0
  name   = "${var.profile}-vpc-endpoint-autoscaling"
  vpc_id = var.vpc_id
  tags   = merge(local.rc_tags, var.tags)
